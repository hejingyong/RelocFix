...
void Decode(DISASSEMBLY *Disasm,  // 存放解析后的信息
            char *Opcode,         // 解析地址
            DWORD *Index)         // 传出解析指令长度
{
    /*
...
{
    //-----------------清空所有解码后指令信息
    strcpy(Disasm->Assembly, "");  //清空反汇编内容
    strcpy(Disasm->Remarks,  "");  //清空注释
    strcpy(Disasm->Opcode,   "");  //清空操作码
    Disasm->OpcodeSize = 1;        //最小化操作码为1个指令
    Disasm->PrefixSize = 0;        //没有前缀
}
